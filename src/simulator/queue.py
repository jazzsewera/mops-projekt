...
class Queue(object):
    ...
    def queue_packet_listener(self, current_time):
        ...
        if self.packets and not self._current_packet:
            ...
            self.log.debug(
                f"Server busy, currently handling packet: {self._current_packet}"
            )
            ...
            if self._queue is not None:
                ...
                self.log.info("###Q2###")
            self.log.info(f"Finished handling packet: {self._current_packet}")
            if self._current_packet.is_passing:
                ...
                if self._queue is not None:
                    ...
                    self.log.info("Sending packet to next queue")
            self._current_packet = None
        else:
